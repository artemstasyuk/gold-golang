...
func (s NewsService) dbSort(ops *ViewOps) db.OpFunc {
	...
	switch ops.SortColumn {
	case db.Columns.News.ID, db.Columns.News.Title, db.Columns.News.Alias, db.Columns.News.Content, db.Columns.News.CategoryID, db.Columns.News.CreatedAt, db.Columns.News.UpdatedAt, db.Columns.News.PublicationDate, db.Columns.News.StatusID:
		v = db.WithSort(db.NewSortField(ops.SortColumn, ops.SortDesc))
	}
...
func (s NewsService) isValid(ctx context.Context, news News, isUpdate bool) Validator {
	...
	item, err := s.newsRepo.OneNews(ctx, search)
	if err != nil {
